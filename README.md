...
> - 요청은 Laravel `ChatController`에서 chatbot-engine(`CHATBOT_ENGINE_URL`)으로 프록시됩니다.
> - 대화 이력은 chatbot-engine의 Redis(`REDIS_URL`)에 세션별로 저장되고, Laravel DB에도 `chat_messages` 테이블로 이중 저장됩니다.
> - `schedule`은 여행 일정 확정 응답(`type: "itinerary"`)일 때만 포함됩니다.
> - `schedule` 각 항목: `day`, `time`, `place`, `latitude`, `longitude`, `description`
...
└── Feature/
    ...
    │   ├── ChatTest.php           # 챗봇 메시지, 이력 조회/삭제 (16개)
...
| `ChatMessageFactory` | 챗봇 대화 메시지 테스트 데이터 (`.assistant()` state 포함) |
...
# 환경 파일 설정
cp .env.example .env
# .env 에 OPENAI_API_KEY, LARAVEL_SERVICE_TOKEN, REDIS_URL 입력
...
| `LARAVEL_SERVICE_TOKEN` | Laravel→chatbot-engine Bearer 인증 토큰 (양쪽 동일 값) | - |
| `REDIS_URL` | Redis 연결 문자열 (세션 영속화) | `redis://localhost:6379` |
...
`LARAVEL_SERVICE_TOKEN`은 **두 곳에 동일한 값**을 설정해야 합니다.

- `TravelPlatform/.env` → `LARAVEL_SERVICE_TOKEN=<토큰>`  (Laravel이 chatbot-engine 호출 시 Bearer 헤더로 첨부)
- `chatbot-engine/.env` → `LARAVEL_SERVICE_TOKEN=<토큰>`  (chatbot-engine이 수신 헤더 검증에 사용)

임의의 긴 문자열을 생성하여 양쪽에 동일하게 설정하면 됩니다:
...
# TravelPlatform/.env
CHATBOT_ENGINE_URL=http://localhost:8001
LARAVEL_SERVICE_TOKEN=<생성된_토큰>
...
> 모든 엔드포인트(`/health` 제외)는 `Authorization: Bearer {LARAVEL_SERVICE_TOKEN}` 헤더가 필요합니다. Laravel `ChatController`가 이 헤더를 자동으로 첨부합니다.
...
POST /chat
  └─ ChatService.chat()
       └─ LangGraph Agent (create_react_agent + RedisSaver)
...
| `REDIS_SCHEME` | `tls` |
| `REDIS_PORT` | `6379` |
| `CHATBOT_ENGINE_URL` | chatbot-engine FastAPI 서버 URL |
| `LARAVEL_SERVICE_TOKEN` | Laravel→chatbot-engine Bearer 인증 토큰 |
| `TOURCAST_API_KEY` | TourCast API 인증 키 |
